...
try:
	...
	print "Twiast ERROR: Module twisted not found."
	print "You need twisted matrix 10.1+ to run Twiast. Get it from http://twistedmatrix.com/"
	sys.exit(1)
...
try:
	...
	print "Twiast ERROR: Module starpy not found."
	print "You need starpy to run Twiast. Get it from http://www.vrplumber.com/programming/starpy/"
	sys.exit(1)
...
## Twiast HTTP
##
class TwiastHTTP(resource.Resource):
	...
	def __init__(self, host, port):
		log.info('Initializing Twiast HTTP Server at %s:%s...' % (host, port))
		self.handlers = {
	...
	def _expireSession(self):
		expired = [sessid for sessid, session in self.sessions.items() if not self.twiast.site.sessions.has_key(sessid)]
	...
	def render_GET(self, request):
		...
		if not self.sessions.has_key(session.uid):
			...
			session.updates            = []
			session.isAuthenticated    = not self.twiast.authRequired
	...
	def doAuthentication(self, request):
		...
		if username != None and secret != None:
			authUser = self.twiast.authUsers.get(username)
			if authUser:
	...
	def getStatus(self, request):
		...
		session    = request.getSession()
		server     = self.twiast.servers.get(servername)
		...
		for tech, peerlist in server.status.peers.items():
			...
			tmp[servername]['peers'][tech].sort(lambda x, y: cmp(x.get(self.twiast.sortPeersBy), y.get(self.twiast.sortPeersBy)))
		## Channels
	...
	def listServers(self, request):
		session = request.getSession()
		servers = self.twiast.servers.keys()
		if self.twiast.authRequired and session.isAuthenticated and session.username:
			servers = self.twiast.authUsers[session.username].servers.keys()
		servers.sort()		
	...
	def doAction(self, request):
		session = request.getSession()
		self.twiast.clientActions.append((session, request.args))
		reactor.callWhenRunning(self.twiast._processClientActions)
		request.write("OK")
		request.finish()
...
## Twiast AMI
##
class TwiastAMIProtocol(manager.AMIProtocol):
...
class TwiastAMIFactory(manager.AMIFactory):
	amiWorker  = None
	servername = None
	protocol   = TwiastAMIProtocol
	def __init__(self, servername, username, password, amiWorker):
		log.info('Server %s :: Initializing Twiast AMI Factory...' % servername)
...
class Twiast:
	...
	def __init__(self, configFile):
		log.log(logging.NOTICE, "Initializing Twiast AMI Interface...")
		...
		self.__parseTwiastConfig()
		
	def __start(self):
		log.info("Starting Twiast Services...")
	...
	def onLoginFailure(self, reason, servername):
		log.error("Server %s :: Twiast AMI Failed to Login, reason: %s" % (servername, reason.getErrorMessage()))
	...
	def _createPeer(self, servername, **kw):
		...
		if server.status.peers.has_key(channeltype):
			...
			log.warning("Server %s :: Channeltype %s not implemented in Twiast.", servername, channeltype)
	...
	def __parseTwiastConfig(self):
		...
		self.http        = TwiastHTTP(self.bindHost, self.bindPort)
		self.http.twiast = self
		...
		for server in servers:
			...
			self.servers[servername].factory          = TwiastAMIFactory(servername, username, password, self)
		...
		for meetme, display in config.items('meetmes'):
			...
			if display == "force":
				self._createMeetme(servername, meetme = meetme, forced = True, _log = "By twiast config")
		...
		for user in users:
			...
			try:
				montasUser          = GenericObject("Twiast User")
...
## Daemonizer
##
#MONAST_PID_FILE = '%s/.twiast.pid' % sys.argv[0].rsplit('/', 1)[0]
MONAST_PID_FILE = '/var/run/twiast.pid'
def createDaemon():
	if os.fork() == 0:
		os.setsid()
		if os.fork() == 0:
	...
	pid = os.getpid()
	print '\nTwiast daemonized with pid %s' % pid
...
def RunTwiast(MM):
	...
	opt.add_option('--config',
		dest    = "configFile",
		default = 'twiast.conf',
		help    = "use this config file instead of twiast.conf"
	)
	opt.add_option('--info',
		dest   = "info",
		action = "store_true",
	...
	opt.add_option('--logfile',
		dest    = "logfile",
		default = "twiast.log",
		help    = "use this log file instead of /var/log/twiast.log"
	)
	opt.add_option('--stop',
		dest   = "stop",
		action = "store_true",
		help   = "stop Twiast (only in daemon mode)"
	)
	...
	if options.stop:
		if os.path.exists(MONAST_PID_FILE):
			...
			print "Twiast stopped..."
			sys.exit(0)
		else:
			print "Twiast is not running as daemon..."
			sys.exit(1)
		sys.exit(2)
